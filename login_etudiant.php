<?php
// Démarre la session PHP. DOIT être la toute première ligne avant tout HTML.
session_start();
...
// Assurez-vous que ce fichier initialise une variable $pdo pour la connexion PDO.
require 'db.php'; 

// Initialiser le tableau de réponse pour le retour JSON.
// Tous les messages (succès, erreur, validation) seront envoyés via ce tableau.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // L'e-mail est utilisé comme identifiant de connexion pour les étudiants.
    // L'input HTML doit avoir 'name="email"'.
    $email = filter_input(INPUT_POST, 'email', FILTER_SANITIZE_EMAIL); 
    // Le mot de passe non haché saisi par l'utilisateur. L'input HTML doit avoir 'name="password"'.
    ...
    if (empty($email) || empty($mot_de_passe_saisi)) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("SELECT id_etudiant,nom,prenom,email, mot_de_passe, nom_utilisateur FROM tb_etudiant WHERE email = ?");
            $stmt->execute([$email]);
            $student = $stmt->fetch(PDO::FETCH_ASSOC); // Récupérer le résultat sous forme de tableau associatif.
            ...
            if ($student) {
                ...
                if (password_verify($mot_de_passe_saisi, $student['mot_de_passe'])) {
                    ...
                    $_SESSION['user_id'] = $student['id_etudiant'];          // ID unique de l'étudiant
                    $_SESSION['user_email'] = $student['email'];            // E-mail de l'étudiant
                    $_SESSION['username'] = $student['nom_utilisateur'];    // Nom d'utilisateur de l'étudiant
                    $_SESSION['user_role'] = 'student';       
                    $_SESSION['user_nom'] = $student['nom'];                 // Définir le rôle comme 'student'
                    $_SESSION['user_prenom'] = $student['prenom'];  

                    $response['success'] = true;
                    $response['message'] = "Connexion réussie ! Bon retour, " . htmlspecialchars($student['nom_utilisateur']) . ".";
                    // Redirection vers le tableau de bord étudiant (fichier PHP, car il vérifie la session).
                    $response['redirect_url'] = 'tableau_de_bord_etudiant.php'; 
                } else {
...
<header class="Header">
    ...
    <main class="contenu login">
        <h2 class="login-form-title">Connexion Étudiant</h2>
        ...
        <form id="loginForm" action="" method="POST" class="stage-form"> 
            ...
            <div class="form-actions">
                        ...
                        <p class="mt-6" >
                        <a href="mot_de_passe_oublie.php" class="login-mdp"> Mot de passe oublié ?  </a>
                        </p>
                        <p class="mt-4" >
                        Vous êtes une nouveau sur la plateforme ? Inscrivez vous !
                        <a href="register_stagiaires.php" class="signun-mdp"> Inscription </a>
                        </p>
            </div>
